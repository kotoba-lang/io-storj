(ns storj.sigv4
  "AWS Signature Version 4 — the **pure** half.
  ...
  vectors — see `test/storj/sigv4_test.cljc`, which asserts the documented
  ...
  here. The host supplies them (`storj.crypto`, or your own `ICrypto`), because
  their shape differs per runtime: `javax.crypto` is synchronous on the JVM
  while `crypto.subtle` returns Promises in a browser / Worker. Keeping them out
  means this namespace stays identical on every runtime in the kotoba-lang
  ladder (kotoba wasm > clojurewasm > cljs > nbb), and stays testable without a
  crypto provider.
...
(def service
  "SigV4 service name. Storj Gateway-MT is an S3-compatible surface, so it signs
  as `s3` like any other S3 endpoint."
  "s3")
...
(defn object-path
  ...
  Path-style (rather than virtual-hosted-style) because it is what Storj's
  own documented `--endpoint-url https://gateway.storjshare.io` examples
  exercise, and it avoids the DNS and TLS-SAN requirements of host-based
  addressing. An empty/blank `key` degrades to `bucket-path`."
  [bucket key]
