(ns storj.crypto
  ...
  Optional: `storj.core` only needs the protocol, so a host with its own crypto
  ...
  WebCrypto returns Promises of bytes. `storj.core/then` absorbs that, so both
  implementations satisfy the same protocol without either pretending to be the
  other."
  (:require [storj.protocols :as p])
